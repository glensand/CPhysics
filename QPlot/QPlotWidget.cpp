	...
	: QMainWindow(parent)
{
	...
	for (const auto& function : plotParams.m_functions)
	{
		...
		m_ui.widget->graph(0)->setPen(color);

		// sets default graph style
		// TODO: add styles convertation
		m_ui.widget->graph(0)->setLineStyle(QCPGraph::lsLine);
		//m_ui.widget->graph(0)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, 4));

		m_ui.widget->xAxis->setLabel(function.m_labels.m_arg.c_str());
		m_ui.widget->yAxis->setLabel(function.m_labels.m_val.c_str());
